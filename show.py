...
labels_0 = ['','happy', 'sad','feared', 'angry', 'disgusted', 'surprise', 'nothing']
labels = ['happy', 'sad','nothing', 'nothing', 'nothing', 'surprise', 'nothing']
#parameters setting
interval = 5
width = 1080
height = 720
	...
	def run(self):  
		...
		if stream.isSuccess:
			...
			while (not stop_sign and stream.isSuccess):
				...
				print("line 83, image.shape=", image.shape)
				count += 1
				...
				#image = cv2.cvtColor(img_resized, cv2.COLOR_RGB2BGR)
				#cv2.imwrite('../%d.jpg'%count,image)
				wx.CallAfter(pub.sendMessage, "update", 
							msg = img_resized.tostring())
				if count%100==0: 
...
class MyForm(wx.Frame):  
	...
	def __init__(self, c, d):  
		...
		# self.ax = axisartist.Subplot(self.Figure, '1’)
		...
		self.rb = wx.RadioBox(self.panel, -1, label="Sources：", pos=wx.DefaultPosition,
	...
	def updateFigure(self, msg):
		...
		if not msg == 'end': 
			...
			# matplotlib.pyplot.xticks(x, labels, rotation=30)
			self.axes.plot(x,y,'--^g')
			# self.axes.xaxis.set_tick_params(rotation=30, labelsize=10)
			self.axes.xaxis.set_major_formatter(FuncFormatter(self.format_fn))
			self.axes.yaxis.set_major_locator(MultipleLocator(5))
			self.axes.set_xticklabels(labels_0, rotation=10)
